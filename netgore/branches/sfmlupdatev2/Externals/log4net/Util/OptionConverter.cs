...
namespace log4net.Util
{
    ...
    public sealed class OptionConverter
    {
        ...
        public static bool CanConvertTypeTo(Type sourceType, Type targetType)
        {
            ...
            var tcSource = ConverterRegistry.GetConvertTo(sourceType, targetType);
            if (tcSource != null)
            {
            ...
            var tcTarget = ConverterRegistry.GetConvertFrom(targetType);
            if (tcTarget != null)
            {
        ...
        public static object ConvertStringTo(Type target, string txt)
        {
            ...
            var typeConverter = ConverterRegistry.GetConvertFrom(target);
            ...
            else
            {
                ...
                else
                {
                    ...
                    var meth = target.GetMethod("Parse", new Type[] { typeof(string) });
                    if (meth != null)
                    {
        ...
        public static object ConvertTypeTo(object sourceInstance, Type targetType)
        {
            ...
            throw new ArgumentException(
                "Cannot convert source object [" + sourceInstance + "] to target type [" + targetType.Name + "]", "sourceInstance");
        }
        ...
        public static object InstantiateByClassName(string className, Type superClass, object defaultValue)
        {
            if (className != null)
            {
                try
                {
                    var classObj = SystemInfo.GetTypeFromString(className, true, true);
                    if (!superClass.IsAssignableFrom(classObj))
                    {
                        LogLog.Error("OptionConverter: A [" + className + "] object is not assignable to a [" +
        ...
        public static string SubstituteVariables(string value, IDictionary props)
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    {
                        j += DELIM_START_LEN;
                        var key = value.Substring(j, k - j);

                        var replacement = props[key] as string;
        ...
        public static long ToFileSize(string argValue, long defaultValue)
        {
            ...
            var s = argValue.Trim().ToUpper(CultureInfo.InvariantCulture);
            long multiplier = 1;
            int index;
        ...
        static object ParseEnum(Type enumType, string value, bool ignoreCase)
        {
