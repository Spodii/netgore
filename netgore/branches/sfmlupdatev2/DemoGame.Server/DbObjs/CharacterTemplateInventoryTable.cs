        ...
        public static ColumnMetadata GetColumnData(String columnName)
        {
            switch (columnName)
            {
                ...
                case "item_template_id":
                    return new ColumnMetadata("item_template_id", "The item the character template has in their inventory.",
                        "smallint(5) unsigned", null, typeof(UInt16), false, false, true);

                case "max":
                    return new ColumnMetadata("max", "The maximum number of items to be created.", "tinyint(3) unsigned", null,
                        typeof(Byte), false, false, false);

                case "min":
                    return new ColumnMetadata("min",
