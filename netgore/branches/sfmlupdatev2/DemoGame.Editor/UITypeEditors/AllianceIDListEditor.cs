...
{
    /// <summary>
    /// A <see cref="UITypeEditor"/> for a list of <see cref="AllianceID"/>s.
    /// </summary>
    public class AllianceIDListEditor : UITypeEditor
    {
        /// <summary>
        ...
        public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value)
        {
            ...
            if (svc != null)
            {
                var v = value as List<AllianceID>;
                if (v == null)
                    return value;

                using (var editorForm = new AllianceIDListForm(v))
                {
