...
{
    /// <summary>
    /// A <see cref="UITypeEditor"/> for selecting the <see cref="CharacterTemplateAndAmountEditor"/>.
    /// </summary>
    public class CharacterTemplateAndAmountEditor : UITypeEditor
    {
        /// <summary>
        ...
        public override object EditValue(ITypeDescriptorContext context, IServiceProvider provider, object value)
        {
            ...
            if (svc != null)
            {
                var v = value as List<MutablePair<CharacterTemplateID, ushort>>;
                if (v == null)
                    return value;

                using (var editorForm = new CharacterTemplateAndAmountListForm(v))
                {
