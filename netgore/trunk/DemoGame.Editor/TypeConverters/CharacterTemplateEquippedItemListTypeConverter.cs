...
{
    /// <summary>
    /// A <see cref="TypeConverter"/> for a collection of <see cref="CharacterTemplateEquippedItem"/>s and values.
    /// </summary>
    public class CharacterTemplateEquippedItemListTypeConverter : TypeConverter
    {
        /// <summary>
        ...
        public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType)
        {
            if (destinationType == typeof(string))
            {
                var ev = value as IEnumerable<CharacterTemplateEquippedItem>;
                if (ev != null)
                {
