...
namespace NetGore.Graphics.GUI
{
    ...
    public class GameControl
    {
        ...
        public void Update(IGUIManager guiManager, int currentTime)
        {
            ...
            if (!GameControlKeys.KeysUp.All(ks.IsKeyUp))
                return;

            if (!GameControlKeys.KeysDown.All(ks.IsKeyDown))
                return;

            if (!GameControlKeys.NewKeysDown.All(guiManager.NewKeysDown.Contains))
                return;

            if (!GameControlKeys.NewKeysUp.All(guiManager.NewKeysUp.Contains))
                return;
