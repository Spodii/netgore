...
namespace DemoGame.Server.Queries
{
    ...
    public class SelectCharacterEquippedItemsQuery : DbQueryReader<CharacterID>
    {
        ...
        static string CreateQuery(IQueryBuilder qb)
        {
            ...
            var f = qb.Functions;
            var s = qb.Settings;
            var q = qb.Select(ItemTable.TableName, "i").AllColumns("i").Add("c.slot")
                .InnerJoinOnColumn(CharacterEquippedTable.TableName, "c", "item_id", "i", "id")
                .Where(f.Equals("c.character_id", s.Parameterize("characterID")));
            return q.ToString();
        }
