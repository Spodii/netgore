...
namespace DemoGame.Server.Queries
{
    ...
    public class SelectAllianceHostileQuery : DbQueryReader<AllianceID>
    {
        ...
        static string CreateQuery(IQueryBuilder qb)
        {
            ...
            var f = qb.Functions;
            var s = qb.Settings;
            var q = qb.Select(AllianceHostileTable.TableName).AllColumns().Where(f.Equals(s.EscapeColumn("alliance_id"), s.Parameterize("id")));
            return q.ToString();
        }
        ...
        public SelectAllianceHostileQuery(DbConnectionPool connectionPool) : base(connectionPool, CreateQuery(connectionPool.QueryBuilder))
        {
        }
