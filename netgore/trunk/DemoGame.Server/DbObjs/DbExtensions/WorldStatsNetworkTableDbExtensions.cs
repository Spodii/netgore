...
namespace DemoGame.Server.DbObjs
{
    ...
    public static class WorldStatsNetworkTableDbExtensions
    {
        ...
        public static void CopyValues(this IWorldStatsNetworkTable source, DbParameterValues paramValues)
        {
            ...
            paramValues["connections_rejected"] = source.ConnectionsRejected;
            paramValues["id"] = source.ID;
            paramValues["recv"] = source.Recv;
            paramValues["recvs"] = source.Recvs;
            paramValues["sends"] = source.Sends;
            paramValues["sent"] = source.Sent;
        ...
        public static Boolean HasSameValues(this IWorldStatsNetworkTable source, IWorldStatsNetworkTable otherItem)
        {
            return Equals(source.Connections, otherItem.Connections) &&
                   Equals(source.ConnectionsRejected, otherItem.ConnectionsRejected) && Equals(source.ID, otherItem.ID) &&
                   Equals(source.Recv, otherItem.Recv) && Equals(source.Recvs, otherItem.Recvs) &&
                   Equals(source.Sends, otherItem.Sends) && Equals(source.Sent, otherItem.Sent) &&
        ...
        public static void ReadValues(this WorldStatsNetworkTable source, IDataReader dataReader)
        {
            ...
            source.ConnectionsRejected = dataReader.GetUInt32(i);
            ...
            source.Recv = dataReader.GetUInt32(i);

            i = dataReader.GetOrdinal("recvs");

            source.Recvs = dataReader.GetUInt32(i);

            i = dataReader.GetOrdinal("sends");

            source.Sends = dataReader.GetUInt32(i);

            i = dataReader.GetOrdinal("sent");

            source.Sent = dataReader.GetUInt32(i);
            ...
            for (var i = 0; i < paramValues.Count; i++)
            {
                switch (paramValues.GetParameterName(i))
                {
                    ...
                    case "connections_rejected":
                        paramValues[i] = source.ConnectionsRejected;
                        break;
                    ...
                    case "recv":
                        paramValues[i] = source.Recv;
                        break;

                    case "recvs":
                        paramValues[i] = source.Recvs;
                        break;

                    case "sends":
                        paramValues[i] = source.Sends;
                        break;

                    case "sent":
                        paramValues[i] = source.Sent;
                        break;
