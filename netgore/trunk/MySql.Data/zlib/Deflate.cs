...
namespace zlib
{
    sealed class Deflate
    {
        ...
        static readonly String[] z_errmsg = new String[]
                                            {
                                                "need dictionary", "stream end", "", "file error", "stream error", "data error",
        ...
        internal int deflate(ZStream pstrm, int flush)
        {
            if (flush > Z_FINISH || flush < 0)
                return Z_STREAM_ERROR;

            if (pstrm.next_out == null || (pstrm.next_in == null && pstrm.avail_in != 0) ||
                (status == FINISH_STATE && flush != Z_FINISH))
            {
                pstrm.msg = z_errmsg[Z_NEED_DICT - (Z_STREAM_ERROR)];
                return Z_STREAM_ERROR;
            }
            if (pstrm.avail_out == 0)
            {
                pstrm.msg = z_errmsg[Z_NEED_DICT - (Z_BUF_ERROR)];
                return Z_BUF_ERROR;
            }

            strm = pstrm; // just in case
            ...
            if (status == INIT_STATE)
            {
                ...
                if (strstart != 0)
                {
                    putShortMSB((int)(SupportClass.URShift(pstrm.adler, 16)));
                    putShortMSB((int)(pstrm.adler & 0xffff));
                }
                pstrm.adler = pstrm._adler.adler32(0, null, 0, 0);
            }
            ...
            else if (pstrm.avail_in == 0 && flush <= old_flush && flush != Z_FINISH)
            {
                pstrm.msg = z_errmsg[Z_NEED_DICT - (Z_BUF_ERROR)];
                return Z_BUF_ERROR;
            }
            ...
            if (status == FINISH_STATE && pstrm.avail_in != 0)
            {
                pstrm.msg = z_errmsg[Z_NEED_DICT - (Z_BUF_ERROR)];
                return Z_BUF_ERROR;
            }
            ...
            if (pstrm.avail_in != 0 || lookahead != 0 || (flush != Z_NO_FLUSH && status != FINISH_STATE))
            {
                int bstate = - 1;
        ...
        internal int deflateInit(ZStream pstrm, int plevel, int bits)
        {
            return deflateInit2(pstrm, plevel, Z_DEFLATED, bits, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY);
        }

        internal int deflateInit(ZStream pstrm, int plevel)
        {
            return deflateInit(pstrm, plevel, MAX_WBITS);
        }

        internal int deflateInit2(ZStream pstrm, int plevel, int pmethod, int windowBits, int memLevel, int pstrategy)
        {
            int lnoheader = 0;
            ...
            if (plevel == Z_DEFAULT_COMPRESSION)
                plevel = 6;
            ...
            if (memLevel < 1 || memLevel > MAX_MEM_LEVEL || pmethod != Z_DEFLATED || windowBits < 9 || windowBits > 15 ||
                plevel < 0 || plevel > 9 || pstrategy < 0 || pstrategy > Z_HUFFMAN_ONLY)
                return Z_STREAM_ERROR;
            ...
            return deflateReset(pstrm);
        }

        internal int deflateParams(ZStream pstrm, int _level, int _strategy)
        {
            int err = Z_OK;
            ...
            if (config_table[level].func != config_table[_level].func && pstrm.total_in != 0)
            {
                // Flush the last buffer:
                err = pstrm.deflate(Z_PARTIAL_FLUSH);
            }
        ...
        internal int deflateReset(ZStream pstrm)
        {
            pstrm.total_in = pstrm.total_out = 0;
            pstrm.msg = null; //
            pstrm.data_type = Z_UNKNOWN;
        ...
        internal int deflateSetDictionary(ZStream pstrm, byte[] dictionary, int dictLength)
        {
            ...
            pstrm.adler = pstrm._adler.adler32(pstrm.adler, dictionary, 0, dictLength);
        ...
        internal int longest_match(int cur_match)
        {
            ...
            if (lnice_match > lookahead)
                lnice_match = lookahead;
