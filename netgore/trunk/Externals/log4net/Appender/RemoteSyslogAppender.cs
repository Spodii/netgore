    ...
    public class RemoteSyslogAppender : UdpAppender
    {
        ...
        protected override void Append(LoggingEvent loggingEvent)
        {
            try
            {
                ...
                var fullMessage = writer.ToString();
                var buffer = Encoding.GetBytes(fullMessage.ToCharArray());
            ...
            {
                ErrorHandler.Error(
                    "Unable to send logging event to remote syslog " + RemoteAddress + " on port " + RemotePort + ".", e,
