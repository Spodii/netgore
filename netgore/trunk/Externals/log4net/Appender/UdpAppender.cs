...
namespace log4net.Appender
{
    ...
    public class UdpAppender : AppenderSkeleton
    {
        ...
        protected override void Append(LoggingEvent loggingEvent)
        {
            try
            {
                var buffer = m_encoding.GetBytes(RenderLoggingEvent(loggingEvent).ToCharArray());
                Client.Send(buffer, buffer.Length, RemoteEndPoint);
            }
            catch (Exception ex)
            {
                ErrorHandler.Error(
                    "Unable to send logging event to remote host " + RemoteAddress + " on port " + RemotePort + ".", ex,
        ...
        protected virtual void InitializeClientConnection()
        {
            ...
            {
                ErrorHandler.Error(
                    "Could not initialize the UdpClient connection on port " + LocalPort.ToString(NumberFormatInfo.InvariantInfo) +
