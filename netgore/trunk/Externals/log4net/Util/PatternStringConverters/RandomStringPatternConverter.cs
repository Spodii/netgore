...
namespace log4net.Util.PatternStringConverters
{
    ...
    sealed class RandomStringPatternConverter : PatternConverter, IOptionHandler
    {
        /// <summary>
        ...
        protected override void Convert(TextWriter writer, object state)
        {
            try
            {
                lock (s_random)
                {
                    ...
                    {
                        var randValue = s_random.Next(36);

                        if (randValue < 26)
                        {
                            // Letter
                            var ch = (char)('A' + randValue);
                            writer.Write(ch);
                        }
                        else if (randValue < 36)
                        {
                            // Number
                            var ch = (char)('0' + (randValue - 26));
                            writer.Write(ch);
                        }
                        else
                        {
