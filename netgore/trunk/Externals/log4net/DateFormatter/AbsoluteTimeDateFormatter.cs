...
namespace log4net.DateFormatter
{
    ...
    public class AbsoluteTimeDateFormatter : IDateFormatter
    {
        ...
        {
            var hour = dateToFormat.Hour;
            if (hour < 10)
                buffer.Append('0');
            buffer.Append(hour);
            buffer.Append(':');

            var mins = dateToFormat.Minute;
            if (mins < 10)
                buffer.Append('0');
            buffer.Append(mins);
            buffer.Append(':');

            var secs = dateToFormat.Second;
            if (secs < 10)
                buffer.Append('0');
            buffer.Append(secs);
        }
        ...
        public virtual void FormatDate(DateTime dateToFormat, TextWriter writer)
        {
            ...
            var currentTimeToTheSecond = (dateToFormat.Ticks - (dateToFormat.Ticks % TimeSpan.TicksPerSecond));
            ...
            if (s_lastTimeToTheSecond != currentTimeToTheSecond)
            {
                ...
                lock (s_lastTimeBuf)
                {
                    if (s_lastTimeToTheSecond != currentTimeToTheSecond)
                    {
                        ...
                        var currentDateWithoutMillis = s_lastTimeBuf.ToString();
            ...
            writer.Write(',');
            var millis = dateToFormat.Millisecond;
            if (millis < 100)
                writer.Write('0');
            if (millis < 10)
                writer.Write('0');
        ...
        /// as a string.
        /// </summary>
        static readonly StringBuilder s_lastTimeBuf = new StringBuilder();

        /// <summary>
        /// Last stored time with precision up to the second, formatted
        /// as a string.
        /// </summary>
        static string s_lastTimeString;

        /// <summary>
        /// Last stored time with precision up to the second.
        /// </summary>
        static long s_lastTimeToTheSecond = 0;
