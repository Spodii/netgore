...
namespace Lidgren.Network
{
    public partial class NetPeer
    {
        ...
        readonly object m_initializeLock = new object();
        readonly NetQueue<NetIncomingMessage> m_releasedIncomingMessages;
        uint m_frameCounter;
        double m_lastHeartbeat;

        AutoResetEvent m_messageReceivedEvent = new AutoResetEvent(false);
        ...
        void ExecutePeerShutdown()
        {
            ...
            lock (m_handshakes)
            {
                foreach (var conn in m_handshakes.Values)
                {
                    conn.Shutdown(m_shutdownReason);
                }
            }
        ...
        void Heartbeat()
        {
            ...
            var delta = dnow - m_lastHeartbeat;

            var maxCHBpS = 1250 - m_connections.Count;
            if (maxCHBpS < 250)
                maxCHBpS = 250;
            if (delta > (1.0 / maxCHBpS)) // max connection heartbeats/second max
            {
                m_frameCounter++;
            ...
            try
            {
                bytesReceived = m_socket.ReceiveFrom(m_receiveBuffer, 0, m_receiveBuffer.Length, SocketFlags.None,
                    ref m_senderRemote);
            }
            ...
            var ipsender = (IPEndPoint)m_senderRemote;
        ...
        void InitializeNetwork()
        {
            lock (m_initializeLock)
            {
                ...
                var boundEp = m_socket.LocalEndPoint as IPEndPoint;
        ...
        void ReceivedUnconnectedLibraryMessage(IPEndPoint senderEndpoint, NetMessageType tp, int ptr, int payloadByteLength)
        {
            NetConnection shake;
        ...
        internal void VerifyNetworkThread()
        {
            ...
            {
                throw new NetException("Executing on wrong thread! Should be library system thread (is " + ct.Name + " mId " +
                                       ct.ManagedThreadId + ")");
            }
        }
    }
