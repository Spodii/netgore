...
namespace NetGore.Graphics.GUI
{
    ...
    public class GameControl
    {
        ...
        public void Update(IGUIManager guiManager, TickCount currentTime)
        {
            ...
            if (_needsReset)
            {
                if (GameControlKeys.NewKeysDown.IsEmpty() || !GameControlKeys.NewKeysDown.All(x => x.Key == SFML.Window.KeyCode.None || guiManager.IsKeyDown(x.Key)))
                {
            ...
            if (GameControlKeys.KeysUp.Any(x => x.Key != SFML.Window.KeyCode.None && guiManager.IsKeyDown(x.Key)))
                return;

            // Check that all of the keys required to be down are down
            if (!GameControlKeys.KeysDown.All(x => x.Key == SFML.Window.KeyCode.None || guiManager.IsKeyDown(x.Key)))
                return;

            if (!GameControlKeys.NewKeysDown.All(x => x.Key == SFML.Window.KeyCode.None || guiManager.IsKeyDown(x.Key)))
                return;
