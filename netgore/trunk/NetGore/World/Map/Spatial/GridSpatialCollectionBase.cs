...
namespace NetGore
{
    ...
    public abstract class GridSpatialCollectionBase : ISpatialCollection
    {
        ...
        {
            return AsImmutable<T>(values.Distinct<T>());
        }
        ...
        public IEnumerable<ISpatial> GetMany(Vector2 p)
        {
            ...
            return AsImmutable<ISpatial>(segment.Where(x => x.Contains(p)));
        }
        ...
        {
            var segments = GetSegments(rect);
            return AsDistinctAndImmutable<ISpatial>(segments.SelectMany(seg => seg.Where(x => x.Intersects(rect))));
        }
        ...
        public IEnumerable<T> GetMany<T>(Vector2 p)
        {
            ...
            return AsImmutable<T>(segment.Where(x => x.Contains(p)).OfType<T>());
        }
        ...
        {
            var segments = GetSegments(rect);
            return AsDistinctAndImmutable<T>(segments.SelectMany(seg => seg.Where(x => x.Intersects(rect)).OfType<T>()));
        }
