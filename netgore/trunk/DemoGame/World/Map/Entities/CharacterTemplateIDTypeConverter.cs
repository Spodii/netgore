...
{
    /// <summary>
    /// A <see cref="TypeConverter"/> for the <see cref="CharacterTemplateID"/> type.
    /// </summary>
    public class CharacterTemplateIDTypeConverter : SimpleTypeConverter<CharacterTemplateID>
    {
        /// <summary>
        ...
        protected override CharacterTemplateID ConvertFromString(string value, out bool wasConverted)
        {
            int i;
            wasConverted = int.TryParse(value, out i);

            if (wasConverted)
                return new CharacterTemplateID(i);

            return new CharacterTemplateID(1);
        }
    }
}