...
namespace log4net.Core
{
    ...
    public sealed class LoggerManager
    {
        ...
        static LoggerManager()
        {
            ...
            catch (SecurityException)
            {
                LogLog.Debug("LoggerManager: Security Exception (ControlAppDomain LinkDemand) while trying " +
                             ...
                             "will not be called automatically when the AppDomain exits. It must be called " + "programmatically.");
            }
            ...
            var appRepositorySelectorTypeName = SystemInfo.GetAppSetting("log4net.RepositorySelector");
            if (appRepositorySelectorTypeName != null && appRepositorySelectorTypeName.Length > 0)
            {
                ...
                {
                    LogLog.Error(
                        "LoggerManager: Exception while resolving RepositorySelector Type [" + appRepositorySelectorTypeName + "]",
                        ex);
                }
                ...
                if (appRepositorySelectorType != null)
                {
                    ...
                    {
                        LogLog.Error(
                            "LoggerManager: Exception while creating RepositorySelector [" + appRepositorySelectorType.FullName +
                            "]", ex);
                    }
                    ...
                    {
                        LogLog.Error("LoggerManager: RepositorySelector Type [" + appRepositorySelectorType.FullName +
                                     "] is not an IRepositorySelector");
                    }
                }
            }
        ...
        {
            foreach (var repository in GetAllRepositories())
            {
                repository.Shutdown();
            }
        }
        ...
        /// </summary>
        static IRepositorySelector s_repositorySelector;
