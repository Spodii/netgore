...
namespace log4net.Core
{
    ...
    public class DefaultRepositorySelector : IRepositorySelector
    {
        ...
        public DefaultRepositorySelector(Type defaultRepositoryType)
        {
            ...
            if (! (typeof(ILoggerRepository).IsAssignableFrom(defaultRepositoryType)))
            {
                throw SystemInfo.CreateArgumentOutOfRangeException("defaultRepositoryType", defaultRepositoryType,
                    "Parameter: defaultRepositoryType, Value: [" + defaultRepositoryType +
        ...
        public ILoggerRepository CreateRepository(Assembly repositoryAssembly, Type repositoryType, string repositoryName,
                                                  bool readAssemblyAttributes)
        {
            ...
            lock (this)
            {
                ...
                if (rep == null)
                {
                    ...
                    var actualRepositoryName = repositoryName;
                    var actualRepositoryType = repositoryType;
        ...
        public ILoggerRepository CreateRepository(string repositoryName, Type repositoryType)
        {
            ...
            lock (this)
            {
                ...
                else
                {
                    ...
                    if (aliasedRepository != null)
                    {
                        ...
                        else
                        {
                            ...
                            LogLog.Error("DefaultRepositorySelector: Failed to alias repository [" + repositoryName +
                                         "] to existing repository [" + aliasedRepository.Name + "]. Requested repository type [" +
                    ...
                    if (rep == null)
                    {
                        LogLog.Debug("DefaultRepositorySelector: Creating repository [" + repositoryName + "] using type [" +
                                     repositoryType + "]");
        ...
        public ILoggerRepository[] GetAllRepositories()
        {
            lock (this)
            {
                var reps = m_name2repositoryMap.Values;
        ...
        public void AliasRepository(string repositoryAlias, ILoggerRepository repositoryTarget)
        {
            ...
            lock (this)
            {
                ...
                if (m_alias2repositoryMap.Contains(repositoryAlias))
                {
                    ...
                    if (repositoryTarget != (m_alias2repositoryMap[repositoryAlias]))
                    {
                        ...
                        throw new InvalidOperationException("Repository [" + repositoryAlias +
                                                            ...
                                                            ((ILoggerRepository)m_alias2repositoryMap[repositoryAlias]).Name +
        ...
        {
            var handler = m_loggerRepositoryCreatedEvent;
            if (handler != null)
        ...
        void ConfigureRepository(Assembly assembly, ILoggerRepository repository)
        {
            ...
            if (repository.Name == DefaultRepositoryName)
            {
                ...
                var repositoryConfigFile = SystemInfo.GetAppSetting("log4net.Config");
                if (repositoryConfigFile != null && repositoryConfigFile.Length > 0)
                {
                    ...
                    {
                        LogLog.Error(
                            "DefaultRepositorySelector: Exception while parsing log4net.Config file path [" + repositoryConfigFile +
                            "]", ex);
                    }
        ...
        void LoadPlugins(Assembly assembly, ILoggerRepository repository)
        {
            ...
            if (configAttributes != null && configAttributes.Length > 0)
            {
                foreach (IPluginFactory configAttr in configAttributes)
                {
                    ...
                    {
                        LogLog.Error("DefaultRepositorySelector: Failed to create plugin. Attribute [" + configAttr + "]", ex);
                    }
                }
            }
        }
